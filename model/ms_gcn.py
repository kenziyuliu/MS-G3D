...
# class MultiScale_GraphConv_Exponentiated(nn.Module):
#     def __init__(self, num_scales, in_channels, out_channels, A_binary, use_mask=True, dropout=0, activation='relu'):
#         super().__init__()
...
#             self.A_res = nn.init.uniform_(nn.Parameter(torch.Tensor(self.A_powers.shape)), -1e-6, 1e-6)
#         # else:
#         #     self.A_res = torch.zeros(self.A_powers.shape)
...
# class MultiScale_GraphConv_Disentangled(nn.Module):
#     def __init__(self, num_scales, in_channels, out_channels, A_binary, use_mask=True, dropout=0, activation='relu'):
#         super().__init__()
#         self.num_scales = num_scales
...
#         self.A_powers = self.A_powers.to(x.device)
#         # self.A_res.to(x.device)
#         # A = self.A_powers.to(x.dtype) + self.A_res.to(x.dtype)
#         A = self.A_powers.to(x.dtype)
#         if self.use_mask:
#             A = A + self.A_res.to(x.dtype)
#         support = torch.einsum('vu,nctu->nctv', A, x)
